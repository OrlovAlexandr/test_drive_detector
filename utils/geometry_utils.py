...
def draw_circle(image: np.ndarray, center: Tuple[int, int]) -> None:
    """Draws a circle on the given image."""
    cv2.circle(image, center, RADIUS, COLOR_YELLOW, -1)


def draw_line(image: np.ndarray, start_point: Tuple[int, int], end_point: Tuple[int, int]) -> None:
    """Draws a line on the given image."""
    cv2.line(image, start_point, end_point, COLOR_YELLOW, LINE_THICKNESS)


def draw_closed_path(image: np.ndarray, vertices: List[Tuple[int, int]]) -> None:
